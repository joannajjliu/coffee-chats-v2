...
module.exports = {
  ...
  readCSV: async function readCSV(person, callback, onHoldID) {
    ...
    const peopleOnHold = [];
    fs.readFile("public/read_write/IDsOnHold.csv", "utf8", (err, data) => {
      peopleOnHold.push(data);
      const onHoldIDs = peopleOnHold[0].split(",");
      if (onHoldID != 0 && !onHoldIDs.includes(onHoldID.toString())) {
        //check is not 0, and does not already exist
      ...
      const stream = fs.createReadStream("public/read_write/original.csv");
      stream
  ...
  addOnHoldPerson: function addOnHoldPerson(ignore, prevData, onHold) {
    ...
    stringify(
      onHoldPeople,
      { header: true, columns: PERSON_COLUMNS },
      (err, output) => {
        if (err) throw err;
        fs.writeFile(
          __dirname + "/public/read_write/peopleOnHold.csv",
          output,
          (err) => {
            if (err) throw err;
  ...
  addPerson: function addPerson(person, previousPeopleData) {
    ...
    stringify(
      ...
      (err, output) => {
        if (err) throw err;
        fs.writeFile(
          __dirname + "/public/read_write/new.csv",
          output,
          (err) => {
            if (err) throw err;
  ...
  createPairs: function createPairs(person = null, prevData, onHold) {
    // create copy of onHold into onHoldIDs
    const onHoldIDs = [];
    onHold.map((id) => onHoldIDs.push(id));
    ...
    findOptimalPairs(onHoldPeople, peopleData, "").then((pair) => {
      ...
      stringify(
        optimalPairs,
        { header: true, columns: PAIR_COLUMNS },
        (err, output) => {
          if (err) throw err;
...
async function findOptimalPairs(filler, prevData, filler2) {
  ...
  originalPeopleArray = originalPeopleArray.filter(
    (person) => person[3].split(",")[0] !== ""
  );
  ...
  while (pairStack.length != maxNumberOfPairs) {
    ...
    while (currM === -1) {
      //currM dosen't exist, therefore apply operations to go up one node:
      ...
      if (currM === -1 && pairStack.length === 0) {
        // reached end of function (no optimal result found):
    ...
    if (pairStack.length > bestSoFar.length) {
      //update bestSoFar pairs' array as we proceed
      bestSoFar = []; //reset
...
function createSinglePair(remainingPeopleArray, nIndex, mIndex) {
  ...
  for (i = mIndex; i < nPersonMatchQueue.length; ++i) {
    //loop thr match queue, starting from mIndex
    if (unmatchedIds.includes(nPersonMatchQueue[i])) {
      //if matchQueue[i] is an unmatched person
...
function createUnmatchedIds(unmatchedPersons) {
  //parameter unmatchedPersons is an array of person objects
